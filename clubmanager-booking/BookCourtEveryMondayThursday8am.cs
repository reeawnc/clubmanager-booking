...
{
    public static class BookCourtEveryMondayThursday8am
    {
        public static CourtManager __courtManager { get; private set; }

        [FunctionName("BookCourtEveryMondayThursday8am")]
        public static async Task RunAsync([TimerTrigger("1 0 9 * * 1,4")] TimerInfo myTimer, ILogger log)
        {            
            try
            {
                log.LogInformation($"C# Timer trigger BookCourtEveryMondayThursday8am function executed at: {DateTime.Now}");
                ...
                using (var client = new HttpClient(handler))
                {
                    ...
                    var cell = await __courtManager.GetCourt2Cell(date, baseAddress, client, log);                    
