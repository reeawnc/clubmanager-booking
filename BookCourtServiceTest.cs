...
{
    public class BookCourtServiceTest
    {
        public static CourtManager __courtManager { get; private set; }

        //"0 30 17 * * *" time every day. use an hour before due to server time
        //"0 */10 * * * *" every 10 mins
        //[FunctionName("BookCourtServiceTest")]
        public static async Task RunAsync([TimerTrigger("0 30 18 * * *")] TimerInfo myTimer, ILogger log)
        {
            try
            {
                log.LogInformation($"C# Timer trigger BookCourtServiceTest function executed at: {DateTime.Now}");
                ...
                using (var client = new HttpClient(handler))
                {
                    ...
                    var cell = await __courtManager.GetCourt2Cell(date, baseAddress, client, log, time: "18:00");
                    log.LogInformation($"Finished GetCourt2Cell");                    
                    log.LogInformation($"cell.ToString(): {cell.ToString()}");
                    log.LogInformation(" ");
                    if (IsCellEmpty(cell))
                    {
                        log.LogError($"cell is empty");
                        log.LogInformation($"cell: {cell}");
                        log.LogInformation($"CourtID: {cell.CourtID}");
                        log.LogInformation($"CourtSlotID: {cell.CourtSlotID}");
